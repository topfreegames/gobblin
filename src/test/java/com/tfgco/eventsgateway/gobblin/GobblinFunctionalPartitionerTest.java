 ...
 */
public class GobblinFunctionalPartitionerTest {
    private static final String SIMPLE_CLASS_NAME = GobblinFunctionalPartitionerTest.class.getSimpleName();
...
//        properties.setProp(GobblinDailyPartitioner.WRITER_PARTITION_COLUMNS, PARTITION_COLUMN_NAME);
        ...
        properties.setProp(ConfigurationKeys.WRITER_PARTITIONER_CLASS, GobblinDailyPartitioner.class.getName());
    ...
    public void testWriter() throws IOException {
        ...
        DateTimeUtils.setCurrentMillisFixed(1420185600000L);
        genericRecordBuilder.set("timestamp", 1420185600000l);
        this.writer.write(genericRecordBuilder.build());

        // This timestamp corresponds to 2015/01/03
        DateTimeUtils.setCurrentMillisFixed(1420272000000L);
        ...
        File outputDir20150101 =
                new File(baseOutputDir, "daily" + Path.SEPARATOR + "year=2015" + Path.SEPARATOR + "month=01" + Path.SEPARATOR + "day=01" + Path.SEPARATOR + FILE_NAME);
        Assert.assertTrue(outputDir20150101.exists());

        File outputDir20150102 =
                new File(baseOutputDir, "daily" + Path.SEPARATOR + "year=2015" + Path.SEPARATOR + "month=01" + Path.SEPARATOR + "day=02" + Path.SEPARATOR + FILE_NAME);
        Assert.assertTrue(outputDir20150102.exists());

        File outputDir20150103 =
                new File(baseOutputDir, "daily" + Path.SEPARATOR + "year=2015" + Path.SEPARATOR + "month=01" + Path.SEPARATOR + "day=03" + Path.SEPARATOR + FILE_NAME);
